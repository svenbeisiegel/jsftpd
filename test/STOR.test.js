...
test('test APPE appends to existing file', { timeout }, async () => {
    const users = [
        {
            username: 'john',
            allowLoginWithoutPassword: true,
    ...
    await promiseSocket.write('STOR appefile')
    content = await promiseSocket.read()
    assert.strictEqual(content.toString().trim(), '150 Opening data channel')

    await promiseDataSocket.write('FIRST')
    dataSocket.end()
    await promiseDataSocket.end()

    content = await promiseSocket.read()
    assert.strictEqual(content.toString().trim(), '226 Successfully transferred "appefile"')
    ...
    await promiseSocket.write('APPE appefile')
    content = await promiseSocket.read()
    assert.strictEqual(content.toString().trim(), '150 Opening data channel')

    await promiseDataSocket.write('SECOND')
    dataSocket.end()
    await promiseDataSocket.end()
    ...
    // Verify combined size (5 + 6 = 11)
    await promiseSocket.write('SIZE appefile')
    content = await promiseSocket.read()
    assert.strictEqual(content.toString().trim(), '213 11')

    await promiseSocket.end()
})

test('test APPE without permission', { timeout }, async () => {
    const users = [
        {
            username: 'john',
            allowLoginWithoutPassword: true,
    ...
    await promiseSocket.write('APPE mytestfile')
    content = await promiseSocket.read()
    assert.strictEqual(content.toString().trim(), '550 Transfer failed "mytestfile"')
