...
class ftpd extends EventEmitter {
    ...
    cleanup () {
        if (fs.existsSync(defaultBaseFolder) && fs.statSync(defaultBaseFolder).isDirectory()) {
    ...
    Handler (main, socket) {
        ...
        const SIZE = function (cmd, arg) {
            ...
            if (!main._useHdl && fs.existsSync(file) && fs.statSync(file).isFile() && main._beginsWith(basefolder, file)) {
        ...
        const RETR = function (cmd, arg) {
            ...
            if (!main._useHdl && !fs.existsSync(file)) {
                main._writeToSocket(socket, '550', ' ', 'File not found', connectionInfo, SocketStateAfterWrite.Open)
            }
            if (((fs.existsSync(file) && fs.statSync(file).isFile()) || main._useHdl) && main._beginsWith(basefolder, file)) {
                dataObj.method = async function (obj) {
        ...
        const RNTO = async function (cmd, arg) {
            ...
            if (!main._useHdl && !fs.existsSync(file) && main._beginsWith(basefolder, file)) {
        ...
        const STAT = function (cmd, arg) {
            if (arg) {
                ...
                if (!main._useHdl && fs.existsSync(target) && main._beginsWith(basefolder, target)) {
        ...
        const MDTM = function (cmd, arg) {
            ...
            if (!main._useHdl && fs.existsSync(file) && fs.statSync(file).isFile() && main._beginsWith(basefolder, file)) {
                ...
                main._writeToSocket(socket, '213', ' ', main._getDateForMLSD(mtime), connectionInfo, SocketStateAfterWrite.Open)
            } else {
                main._writeToSocket(socket, '550', ' ', 'File not found', connectionInfo, SocketStateAfterWrite.Open)
            }
        }
        ...
        const authenticatedFunc = {
            QUIT, PWD, CLNT, PBSZ, OPTS, PROT, FEAT, CWD, SIZE, DELE,
            RMD, RMDA: RMD, MKD, LIST, MLSD: LIST,
            PORT, PASV, EPRT, EPSV, RETR, REST, STOR, APPE,
            SYST, TYPE, RNFR, RNTO, MFMT, MDTM,
            NOOP, MODE, STRU, ABOR, CDUP, STAT
        }
        ...
        const setUserRights = function (obj) {
            if (Object.hasOwn(obj, 'basefolder') && fs.existsSync(obj.basefolder) && fs.statSync(obj.basefolder).isDirectory()) {
        ...
        const isValidFolder = function (folder) {
            return fs.existsSync(folder) && fs.statSync(folder).isDirectory() && main._beginsWith(basefolder, folder)
        }
    }
