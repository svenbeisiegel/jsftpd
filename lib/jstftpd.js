...
class tftpd extends EventEmitter {
    ...
    start () {
        ...
        this._socket.on('listening', () => {
            ...
            this.emit('listen', { protocol: 'udp', address: addr.address.replace(/^::ffff:/g, ''), port: addr.port })
        })
    ...
    _handleMessage (msg, rinfo) {
        ...
        const connectionInfo = `[${tftpd._prettyAddr(rinfo.address)}] [${rinfo.port}]`
    ...
    async _handleRRQ (req, rinfo) {
        ...
        if (this._useHdl) {
            let result
            try {
                result = await this._opt.hdl.download(filename)
            } catch (err) {
            ...
            if (Buffer.isBuffer(result)) {
                ...
                stream.end(result)
                this.LogHandler(`${connectionInfo} > serving file="${tftpd._sanitize(filename)}" (${result.length} bytes) via handler`)
                this._startReadTransfer(stream, result.length, rinfo, req.options)
            } else if (result instanceof Readable) {
                this.LogHandler(`${connectionInfo} > serving file="${tftpd._sanitize(filename)}" via handler stream`)
                this._startReadTransfer(result, 0, rinfo, req.options)
            } else {
    ...
    async _handleWRQ (req, rinfo) {
        ...
        if (this._useHdl) {
            let stream
            try {
                stream = await this._opt.hdl.upload(filename)
            } catch (err) {
                this.DebugHandler(`${connectionInfo} handler upload error: ${err.message}`)
                this._sendError(this._socket, ErrorCode.NOT_DEFINED, 'Internal error', rinfo.port, rinfo.address)
                return
            }
            ...
            this._startWriteTransfer(stream, rinfo, req.options)
            return
        }
    ...
    _startReadTransfer (stream, fileSize, rinfo, options) {
        ...
        const remoteAddr = tftpd._prettyAddr(rinfo.address)
        let retries = 0
        let timeout = null
        ...
        const sendOACK = () => {
            ...
            timeout = setTimeout(() => {
                ...
                if (retries > TFTP_MAX_RETRIES) {
                    this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] OACK timed out after ${TFTP_MAX_RETRIES} retries`)
                    cleanup()
                } else {
                    sendOACK()
                }
        ...
        const sendWindow = () => {
            ...
            timeout = setTimeout(() => {
                retries++
                if (retries > TFTP_MAX_RETRIES) {
                    this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] transfer timed out after ${TFTP_MAX_RETRIES} retries`)
                    cleanup()
                } else {
                    sendWindow()
                }
        ...
        const fillAndSendWindow = () => {
            ...
            if (streamEnded === true) {
                if (lastChunkLength === blksize) {
                    ...
                    this.LogHandler(`[${remoteAddr}] [${rinfo.port}] transfer complete (${fileSize} bytes, blksize=${blksize}, wndsize=${windowSize})`)
                    cleanup()
                }
            } else {
        ...
        socket.on('message', (msg, senderInfo) => {
            ...
            if (opcode === Opcode.ERROR) {
                this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] client sent error, aborting transfer`)
                cleanup()
                return
            }
        ...
        socket.on('error', (err) => {
            this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] transfer socket error: ${err.message}`)
            cleanup()
        })
    ...
    _startWriteTransfer (stream, rinfo, options) {
        const socket = dgram.createSocket('udp6')
        const transferKey = ++this._lastTransferKey
        const remoteAddr = tftpd._prettyAddr(rinfo.address)
        let retries = 0
        ...
        const startTimeout = () => {
            timeout = setTimeout(() => {
                retries++
                if (retries > TFTP_MAX_RETRIES) {
                    this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] write transfer timed out after ${TFTP_MAX_RETRIES} retries`)
                    cleanup()
                } else {
        ...
        socket.on('message', (msg, senderInfo) => {
            ...
            if (opcode === Opcode.ERROR) {
                this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] client sent error, aborting write transfer`)
                cleanup()
                return
            }
            ...
            if (data.length < blksize) {
                stream.end(() => {
                    this.LogHandler(`[${remoteAddr}] [${rinfo.port}] write transfer complete (blksize=${blksize})`)
                    cleanup()
                })
                return
            }
        ...
        socket.on('error', (err) => {
            this.DebugHandler(`[${remoteAddr}] [${rinfo.port}] write transfer socket error: ${err.message}`)
            cleanup()
        })
    ...
    static _prettyAddr (addr) {
        return typeof addr === 'string' ? addr.replace(/^::ffff:/g, '') : ''
    }
